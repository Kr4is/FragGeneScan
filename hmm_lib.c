  ...
  for (t=0; t<len_seq; t++){
	...
	if (codon_start==1){
	  ...
	  if(refine) { //add refinement of the start codons here, Ye, April 16, 2016
		...
		if(s == 0) { e_save = freq_sum; s_save = 0; }
		else if(freq_sum < e_save) { e_save = freq_sum; s_save = -1 * s; } //positive chain, upstream s_save = -1 * 3
	  ...
	  */
	  ...
	  if(refine) { //add refinement of the start codons here, Ye, April 16, 2016
		...
		else if(freq_sum < e_save) { e_save = freq_sum; s_save = s; } //negative chain, s_save = s, add, YY July 2018
	      }
	      s += 3;
	      codon[0] = 0;
